...
export default class IndexHeader extends React.PureComponent<IndexHeaderProps, any> {
  ...
  onHelp = (sport: string) => {
    ...
    this.props.dispatch({ type: 'indexHeader/save', payload: { sport } });
          ...
          <Popover
            ...
            content={
              <div className={style.helpList}>
                <p className={style.help} onClick={this.navSport}>
                  体育规则
                </p>
                <Help />
                <p className={style.help} onClick={this.navRule}>
                  规则与条款
                </p>
                <p className={style.help} onClick={this.navOdds}>
                  赔率计算列表
                </p>
              </div>
            }
